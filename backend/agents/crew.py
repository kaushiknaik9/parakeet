...
def _get_agents(variant="primary"):
    """
    Build (and cache) the agent pool for a given LLM variant.
    variant="primary"  -> built from LLM_PROVIDER (e.g. gemini)
    variant="fallback" -> built from LLM_FALLBACK_PROVIDER (e.g. grok)
    """
    if variant not in _agent_pools:
        if variant == "fallback":
            llm = llm_config_fallback(temp=0.15)
            if llm is None:
        ...
        _agent_pools[variant] = _agent_pool
    return _agent_pools[variant]
...
def analyze_deal(transcript: str, timeout_seconds: int = 60):
    ...
    def _kickoff(variant="primary"):
        agents = _get_agents(variant)
